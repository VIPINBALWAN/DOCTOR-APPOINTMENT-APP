...
class PatientRegisterActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnConfirm.setOnClickListener {
            ...
            if (!ClientEmail.contains("@gmail")) {
                Toast.makeText(this, "Enter a valid patient email address", Toast.LENGTH_LONG).show()
                ...
                auth.createUserWithEmailAndPassword(ClientEmail, ClientPassword)
                    .addOnCompleteListener(PatientRegisterActivity()) { task ->
                        ...
                        if (task.isSuccessful) {
                            ...
                            val client = PatientData(user!!.uid, ClientName, ClientSurname, ClientAge, ClientEmail, ClientPassword, "",
                            )
                            db.collection("patients").document(user?.email!!).set(client)
                                .addOnSuccessListener {
                                    Log.d("Firestore", "Client DocumentSnapshot successfully written!")
                                }
